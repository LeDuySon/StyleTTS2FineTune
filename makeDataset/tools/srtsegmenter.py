...
output_dir = os.path.join(root_path, 'segmentedAudio')  # path to where you want the segmented audio to go. dont touch this unless youre having issues
bad_audio_dir = os.path.join(root_path, 'badAudio') # path to where you want the bad audio to go. dont touch this unless youre having issues
srt_dir = os.path.join(root_path, 'srt')
audio_dir = os.path.join(root_path, 'audio')

training_data_path = os.path.join(root_path, 'trainingdata')
...
srt_list = glob.glob(os.path.join(srt_dir, "*.srt")) # Gets a list of all srt files
audio_list = glob.glob(os.path.join(audio_dir, "*.wav")) # Gets a list of all audio files
...
'''
buffer_time = 800  # adds an amount of time in ms to that creates a buffer zone around the subtitles
...
for sub_file in tqdm(srt_list):  # Iterate over all srt files
    ...
    with open(os.path.join(training_data_path, "output.txt"), 'a+') as out_file:
        last_end_time = 0
    ...
    with open(os.path.join(training_data_path, "output.txt"), 'a+') as out_file:
        for i, sub in enumerate(subs):
            ...
            if i < len(subs) - 1:
                ...
                # If gap to next subtitle is larger than the max allowed gap then we add buffer time to the end time
            ...
            if 1850 <= duration <= 30000:   # Adjust these values to your desired range
